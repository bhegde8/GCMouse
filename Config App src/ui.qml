...
ApplicationWindow {
    ...
    //Gets the name of the key based on the given integer code of the key
    function getKeyPress(keyCode) {
        var keyName = chooserListArray[keyCodes.indexOf(keyCode)];
        return keyName;
    }

    //Fixes various errors with the button map chooser text field inputs
    function sanitize(text) {
        if(text[text.length - 1] === text[text.length - 2]) //Takes care of the last character being doubled for A-Z and 0-9 keys
        {
            text = text.substring(0, text.length - 1);
        }
        if(text === "kb_backspac") { //Takes care of the backspace key deleting the last character from the text field
            text = "kb_backspace";
        }

        text = text.replace(/[^0-9a-zA-Z_]/g, ""); //Removes other special characters
        return text.trim();
    }

    //Sets the text of the text field based on the mouse control option chosen from the popup
    ...
    Connections {
        ...
        onApplyJsonConfig: {
            ...
            //This code sets the text of each text field
